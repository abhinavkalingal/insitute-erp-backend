"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PrismaService = void 0;
const common_1 = require("@nestjs/common");
const client_1 = require("@prisma/client");
const request_context_1 = require("../../core/context/request.context");
const prisma_tenant_service_1 = require("./prisma-tenant.service");
const pg_1 = require("pg");
const adapter_pg_1 = require("@prisma/adapter-pg");
let PrismaService = class PrismaService extends client_1.PrismaClient {
    tenantService;
    constructor(tenantService) {
        const pool = new pg_1.Pool({ connectionString: process.env.DATABASE_URL || 'postgresql://postgres:abhi9072@localhost:5432/institute_erp_tenant' });
        const adapter = new adapter_pg_1.PrismaPg(pool);
        super({ adapter });
        this.tenantService = tenantService;
        return new Proxy(this, {
            get: (target, prop) => {
                if (typeof prop !== 'string') {
                    return target[prop];
                }
                const ignoredProps = [
                    'onModuleInit', 'onModuleDestroy',
                    'onApplicationBootstrap', 'onApplicationShutdown', 'beforeApplicationShutdown',
                    'then', 'catch', 'finally',
                    'hasOwnProperty', 'toString', 'valueOf', 'constructor',
                    'isPrototypeOf', 'propertyIsEnumerable', 'toLocaleString'
                ];
                if (typeof prop === 'string') {
                    if (prop === '$connect' || prop === '$disconnect') {
                        return async () => { };
                    }
                    if (ignoredProps.includes(prop) || prop.startsWith('__')) {
                        const val = target[prop];
                        return typeof val === 'function' ? val.bind(target) : val;
                    }
                }
                console.log('Proxy accessed prop:', prop);
                const context = request_context_1.requestContext.getStore();
                if (!context || !context.tenantDbUrl) {
                    throw new common_1.InternalServerErrorException('Tenant context is missing or database URL is not set');
                }
                const client = this.tenantService.getClient(context.tenantDbUrl);
                const value = client[prop];
                if (typeof value === 'function') {
                    return value.bind(client);
                }
                return value;
            }
        });
    }
};
exports.PrismaService = PrismaService;
exports.PrismaService = PrismaService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_tenant_service_1.PrismaTenantService])
], PrismaService);
//# sourceMappingURL=prisma.service.js.map